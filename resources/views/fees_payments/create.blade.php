<x-app-layout>
    ...
    <div class="py-8 max-w-3xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6">
            <form action="{{ route('fees_payments.store') }}" method="POST">
                @csrf
...
<div class="mb-4">
    ...
    <select name="member_id" required
            ...
            @if(request('member_id')) disabled @endif>
        <option value="">Select member...</option>
        @foreach($members as $m)
            <option value="{{ $m->member_id }}"
                {{ (request('member_id') == $m->member_id) ? 'selected' : '' }}>
                {{ $m->full_name }}
            </option>
        @endforeach
    </select>
...
{{-- If the select is disabled, send the member_id manually --}}
...
@endif
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <input type="datetime-local" name="payment_date" value="{{ now()->format('Y-m-d\TH:i') }}"
                               class="w-full border-gray-300 rounded dark:bg-gray-700 dark:text-gray-100" required>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="reference_no" class="w-full border-gray-300 rounded dark:bg-gray-700 dark:text-gray-100">
                    </div>-->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Remarks</label>
                        <input type="text" name="remarks" class="w-full border-gray-300 rounded dark:bg-gray-700 dark:text-gray-100">
                    </div>
                </div>

                <div class="flex justify-end">
                    <a href="{{ route('fees_payments.index') }}" class="text-gray-600 hover:underline mr-4">Cancel</a>
